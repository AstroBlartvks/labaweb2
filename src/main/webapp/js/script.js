...
function drawArea() {
    ...
    ctx.lineTo(CENTER_X + r * SCALE, CENTER_Y - r * SCALE);
    ...
    ctx.beginPath();
    ctx.arc(CENTER_X, CENTER_Y, r2 * SCALE, 0, Math.PI / 2);
...
function validateForm() {
    ...
    if (xValue === '') {
        ...
        if (isNaN(x) || x < -5 || x > 5) {
            xError.textContent = 'X должен быть числом от -5 до 5';
            return false;
        } else {
            xError.textContent = '';
        }
    ...
    if (rSelect.value === '') {
        ...
        return false;
    } else if (!parseFloat(rSelect.value) || parseFloat(rSelect.value) <= 0) {
        rError.textContent = 'Выберите радиус R';
        return false;
    } else {
        rError.textContent = '';
